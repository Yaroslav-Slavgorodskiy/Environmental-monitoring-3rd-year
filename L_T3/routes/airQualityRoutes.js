...
router.get('/station/:stationId', async (req, res) => {
  try {
    const record = await AirQuality.findOne({ station: req.params.stationId }).populate('station');
    if (!record) return res.status(404).json({ message: 'Дані для цієї станції не знайдено' });
    res.json(record);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
